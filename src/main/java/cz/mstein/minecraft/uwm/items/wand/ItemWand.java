...
public class ItemWand extends UWMItem {
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		...
		WandInfo wandInfo = new WandInfo(itemstack, player);
		if(wandInfo.getSelected() == null) {return new ActionResult<>(EnumActionResult.FAIL, itemstack);}
		wandInfo.getSelected().exec(world, player, hand, wandInfo.getMode());
		ItemWand.updateName(wandInfo);
	...
	public static void updateName(WandInfo wandInfo) {
	...
	public static NBTTagCompound setupTags(ItemStack itemStack) {
		...
		if(!tag.hasKey("list", Constants.NBT.TAG_COMPOUND)) {
			...
			String[] gadgetArray = tag.getCompoundTag("list").getKeySet().toArray(new String[1]);
	...
	public static void changeMode(ItemStack itemstack, boolean isUp) {
		...
		WandGadget gadget = WandGadget.getByName(wandInfo.getSelected().getNewMode(wandInfo.getMode(), isUp));
		if(gadget == null) {
			return;
		}
		wandInfo.setSelected(gadget);
	}
}